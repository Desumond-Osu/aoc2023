...
patterns.forEach(pattern => {
    for (const type of ['horizontal', 'vertical']) {
        if (type == 'vertical') {
            pattern = pattern[0].split('').map((_, colIndex) => pattern.map(row => row[colIndex]).join('')); //transpose pattern
        }

        horiLoop: for (let i = 0; i < pattern.length - 1; i++) {
...
function smudgeIndex(p1, p2) {
    ...
    for (let i = 0; i < p1.length; i++) {
        if (p1[i] != p2[i]) {
            if (p1.substring(i+1) == p2.substring(i+1)) {
                return i;
            }
            return -1;
        }
...
for (var pattern of patterns) {
    ...
    for (const type of ['horizontal', 'vertical']) {
        ...
        horiLoop: for (let i = 0; i < pattern.length - 1; i++) {
            ...
            if (pattern[i] != pattern[i+1] && smudgeIndex(pattern[i], pattern[i+1]) == -1) {
                continue;
            }
